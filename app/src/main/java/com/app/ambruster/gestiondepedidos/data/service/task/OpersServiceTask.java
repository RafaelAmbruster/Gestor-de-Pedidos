...
public class OpersServiceTask {

    OpersServiceTaskCallBack callBack;

    public OpersServiceTask(OpersServiceTaskCallBack callBack) {
    ...
    public void CallService(final adUser user, final int page, final String search) {
        ...
        StringRequest strReq = new StringRequest(Request.Method.POST,
                ServiceManager.getInstance().getURL(19).toString(), new Response.Listener<String>() {
                ...
                try {
                    ...
                    if (jObj.getString("success").equals("true")) {
                        ...
                        if (ja.length() > 0) {
                            ...
                            callBack.onOpersServiceTaskCallBack(list);

                            if (ja.length() < ServiceManager.LIMIT) {
                                ArticleListFragment.loadmore = false;
                            }
                        } else {
                            callBack.onOpersServiceTaskCallBack(new ArrayList<adArticle>());
                            ArticleListFragment.loadmore = false;
                        }
                    } else {
                        callBack.onOpersServiceTaskCallBack(new ArrayList<adArticle>());
    ...
    public interface OpersServiceTaskCallBack {
        void onOpersServiceTaskCallBack(ArrayList<adArticle> articles);
